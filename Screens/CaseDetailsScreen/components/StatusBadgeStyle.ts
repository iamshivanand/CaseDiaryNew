...
export const getStatusBadgeStyles = (theme: Theme) => {
  ...
  return StyleSheet.create({
    ...
    open: {
      backgroundColor: isDark ? '#064E3B' : (theme.colors.statusOpenBg || STATUS_GREEN_BG_FALLBACK),
    },
    openText: {
      color: isDark ? '#6EE7B7' : (theme.colors.statusOpenText || STATUS_GREEN_TEXT_FALLBACK),
    },
    inProgress: {
      backgroundColor: isDark ? '#1E3A8A' : (theme.colors.statusInProgressBg || STATUS_BLUE_BG_FALLBACK),
    },
    inProgressText: {
      color: isDark ? '#93C5FD' : (theme.colors.statusInProgressText || STATUS_BLUE_TEXT_FALLBACK),
    },
    closed: {
      backgroundColor: isDark ? '#7F1D1D' : (theme.colors.statusClosedBg || STATUS_RED_BG_FALLBACK),
    },
    closedText: {
      color: isDark ? '#FCA5A5' : (theme.colors.statusClosedText || STATUS_RED_TEXT_FALLBACK),
    },
    onHold: {
      backgroundColor: isDark ? '#78350F' : (theme.colors.statusOnHoldBg || STATUS_YELLOW_BG_FALLBACK),
    },
    onHoldText: {
      color: isDark ? '#FDE68A' : (theme.colors.statusOnHoldText || STATUS_YELLOW_TEXT_FALLBACK),
    },
    appealed: {
      backgroundColor: isDark ? '#312E81' : (theme.colors.statusAppealedBg || STATUS_BLUE_BG_FALLBACK),
    },
    appealedText: {
      color: isDark ? '#C7D2FE' : (theme.colors.statusAppealedText || STATUS_BLUE_TEXT_FALLBACK),
    },
    defaultStatus: {
      backgroundColor: isDark ? '#334155' : (theme.colors.border || DEFAULT_STATUS_GRAY_BG),
    },
    defaultStatusText: {
      color: isDark ? '#CBD5E1' : (theme.colors.textSecondary || DEFAULT_DARK_TEXT),
    },
  });
};
