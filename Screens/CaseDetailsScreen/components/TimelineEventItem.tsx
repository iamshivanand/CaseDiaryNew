...
const TimelineEventItem: React.FC<TimelineEventItemProps> = ({
  ...
  const isDateFeePayment =
    event.event_type === "date_fee_payment" ||
    (event.description || "").includes("(Date Fee)");
  const isTotalFeePayment =
    event.event_type === "total_fee_payment" ||
    (event.description || "").includes("(Total Retainer)");
  const isDateFeeAgreed =
    event.event_type === "date_fee_agreed" ||
    (event.description || "").includes("Date Hearing Fee Agreed");
  ...
  return (
    <View style={styles.rowContainer}>
      ...
      <View
        style={[
          styles.contentBox,
          isPaymentEvent ? { borderColor: badge.border, borderWidth: 1 } : {},
        ]}
      >
        <View
          style={{
        ...
        <Text style={styles.descriptionText}>
          {event.description || t("timeline_no_desc")}
        </Text>
      </View>
    </View>
  );
