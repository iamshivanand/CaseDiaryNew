...
jest.mock("expo-document-picker", () => ({
  ...
  ), // Default mock for cancelled picking
}));
...
jest.mock("expo-file-system", () => ({
  ...
  getInfoAsync: jest.fn((uri) => {
    ...
    if (
      uri.includes("CaseDiaryTestDb") ||
      (typeof uri === "string" &&
        uri.startsWith("file:///mockDocumentDirectory/documents/"))
    ) {
...
jest.mock(
  ...
  () => ({
    printToFileAsync: jest.fn(() =>
      Promise.resolve({ uri: "file:///mock/print/file.pdf" })
    ),
  }),
  { virtual: true }
);
  ...
  () => {
    ...
    const BannerAd = (props) => {
      return React.createElement(
        View,
        { style: props.style },
        React.createElement(Text, null, "BannerAd")
      );
    };
