...
describe("ecourtsParser utils", () => {
  ...
  describe("ecourtsParserJS extraction script & parseRawECourtsData mapping", () => {
    ...
    it("should successfully parse case details from standard eCourts tables", () => {
      ...
      mockHeadings.push({
        innerText: "High Court of Delhi",
        textContent: "High Court of Delhi",
      });
        ...
        createMockTable([
          {
            label: "CNR Number",
            value:
              "   UPBR 01003153 2014 (Note the CNR number for future reference)   ",
          },
        ])
      );
    ...
    it("should extract details from multi-column rows", () => {
      const createMockRowWithCells = (cellValues: string[]) => {
        const mockCells = cellValues.map((val) => ({
          innerText: val,
          textContent: val,
        }));
        return {
    ...
    it("should parse Acts and Sections from tabular Acts tables", () => {
      ...
      const parsedData = parseRawECourtsData(payload.data);
      expect(parsedData.Undersection).toBe(
        "308ipc (Criminal Law Amendment Act)"
      );
    });
    ...
    it("should parse court name from headings with various keywords", () => {
      mockTables.push(
        createMockTable([{ label: "CNR Number", value: "UPBR010031532014" }])
      );

      mockHeadings.push({
        innerText: "District and Session Judge",
        textContent: "District and Session Judge",
      });
  ...
  describe("parseECourtsTxtFile", () => {
    it("should parse legacy character-delimited records correctly", () => {
      const text =
        "UPBR010031532014 | State vs Gopendra | 15-07-2026 | XIII th ADJ";
    ...
    it("should parse official eCourts JSON array backup files correctly", () => {
      ...
      expect(cases[0].court_name).toBe(
        "XIII th ADJ, District and Sessions Judge"
      );
  ...
  describe("checkDuplicateCases", () => {
    const existing = [
      {
        CNRNumber: "UPBR010031532014",
        case_number: "100355/2014",
        court_name: "XIII th ADJ",
      },
  ...
  describe("checkDuplicateAndDiffCases", () => {
    ...
    it("should detect updates when database fields differ from parsed case", () => {
      ...
      expect(result[0].changes?.NextDate).toEqual({
        oldValue: "2026-07-15",
        newValue: "2026-07-20",
      });
      expect(result[0].changes?.CaseNotes).toEqual({
        oldValue: "old note",
        newValue: "new note",
      });
    });
