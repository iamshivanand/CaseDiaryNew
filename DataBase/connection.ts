...
export const getDrizzleDb = async (): Promise<
...
export const resetDbInstance = async (): Promise<void> => {
  ...
  if (dbInstance) {
    try {
      ...
      console.log(
        "DataBase/connection.ts: Database connection closed successfully."
      );
    } catch (error) {
      console.error(
        "DataBase/connection.ts: Failed to close database connection:",
        error
      );
    }
  }
...
export const __TEST_ONLY_resetDbInstance = () => {
  if (dbInstance) {
    dbInstance
      .closeAsync()
      .catch((err) => console.error("Error closing in test reset:", err));
  }
  dbInstance = null;
  drizzleDb = null;
};
...
export const getDb = async (): Promise<SQLite.SQLiteDatabase> => {
  if (dbInstance === null) {
    console.log(
      "DataBase/connection.ts: dbInstance is null, opening new DB connection."
    );
    try {
      ...
      console.log(
        "DataBase/connection.ts: Schema initialized and initial data seeded."
      );
    } catch (error) {
      console.error(
        "DataBase/connection.ts: Failed to open or initialize database:",
        error
      );
  ...
  if (!dbInstance) {
    ...
    throw new Error(
      "DataBase/connection.ts: Database instance is critically null after attempting to open."
    );
  }
  return dbInstance;
};
